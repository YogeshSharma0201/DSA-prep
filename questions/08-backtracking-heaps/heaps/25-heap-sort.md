...
## 1. Linear Time O(n) Heap Creation

### The Concept (Bottom-Up Heapify)
To build a heap from an unsorted array of size $n$, we do **not** insert elements one by one using `push()` (which takes O(n log n)). Instead, we build the heap **bottom-up**:
...
- Total work done:
  $$\text{Total Steps} = \sum_{k=0}^{\lfloor \log n \rfloor} \left\lceil \frac{n}{2^{k+1}} \right\rceil \cdot k \le n \sum_{k=0}^{\infty} \frac{k}{2^{k+1}} = n \cdot 1 = O(n)$$

---

### C++ Priority Queue O(n) Initialization Patterns

#### A. Max-Heap (Default) in O(n)
```cpp
// From std::vector
...
1. **Build Max-Heap (In-Place)**:
   - Convert the unsorted array into a max-heap in O(n) by calling `heapifyDown` from index $n/2 - 1$ down to $0$.
...
| **Heap Sort Extraction ($n-1$ iterations)** | O(n log n) | O(1) |
| **Total Heap Sort** | O(n log n) | O(1) (In-place) |
